...
class BlazeRepository extends Repository
{
    use LogSeamless;
    ...
    public function __construct($method, $debug, App $app)
    {
        ...
        $this->apiId = config($this->method.'.'.$game.'.apiId');

        $this->apiKey = config($this->method.'.'.$game.'.apiKey');
    ...
    public function gameList_($product_id): array
    {
        ...
        $sessionKey = time().rand(1000, 9999);
        $hashKey = md5($this->apiId.'-'.$this->apiKey.'-'.$sessionKey);
        ...
        $response = $this->GameCurl($param, 'gameList');

        if ($response['success'] === true) {

            $return['success'] = true;
            $return['msg'] = $response['msg'];
            $return['games'] = $response['gameList'];

            //            $path = storage_path('logs/seamless/hotdog_game_'.now()->format('Y_m_d').'.log');
            //                file_put_contents($path, print_r($param, true), FILE_APPEND);
            ...
            foreach ($return['games'] as $item) {
                ...
                GameListProxy::updateOrCreate(
                    ['product' => $product_id, 'code' => $item['gameId']],
                    ['category' => 'EGAMES', 'type' => 'SLOT', 'img' => $item['iconUrl'], 'name' => $item['gameName'], 'rank' => 1, 'game' => $item['gameId']]
                );
            }
    ...
    {
        $productId = strtoupper(trim($product_id));
        $result    = ['success' => false, 'msg' => 'Unknown error', 'games' => []];

        // --- สร้างพารามิเตอร์ตามสเปกเดิม ---
        $sessionKey = time() . rand(1000, 9999);
        $hashKey    = md5($this->apiId . '-' . $this->apiKey . '-' . $sessionKey);
        ...
        if (($response['success'] ?? false) !== true) {
            $result['msg']     = $response['msg'] ?? 'Provider error';
            $result['success'] = false;
            return $result;
        }

        // --- แปลงผลลัพธ์เป็นรูปแบบกลางของระบบเรา ---
        $rawList = (array)($response['gameList'] ?? []);
        $games   = [];

        foreach ($rawList as $item) {
            // map: gameId, gameName, iconUrl -> code, name, img
            $code = (string)($item['gameId'] ?? '');
            if ($code === '') {
                continue; // ข้าม item ที่ไม่มีรหัสเกม
            }

            $games[] = [
                'code'     => $code,
                'name'     => (string)($item['gameName'] ?? $code),
                'img'      => (string)($item['iconUrl'] ?? ''),
                'category' => 'EGAMES',   // คงค่าตามเดิมของคุณ
                'type'     => 'SLOT',     // คงค่าตามเดิมของคุณ
                'rank'     => 1,          // คงค่าตามเดิมของคุณ
            ];
        }

        // --- ซิงก์เข้า Mongo: upsert + ปิดเกมที่หายไปจาก API รอบนี้ ---
        /** @var \Gametech\Game\Services\GameListSyncService $sync */
        $sync = app(\Gametech\Game\Services\GameListSyncService::class);
        $sync->syncFromApi($productId, $games, ['disable_missing' => true]);

        // --- คืนค่าแบบเดิม ---
        $result['success'] = true;
        $result['msg']     = $response['msg'] ?? 'OK';
        $result['games']   = $games;
...
//        $member = DB::table('members')->select('code', 'user_name', 'balance')->where('session_id', request()->session()->getId())->first();
        ...
        if ($member) {
            ...
            $response = $this->GameCurl($param, 'gameLaunch');

            $response['param'] = $param;
            $response['datetime'] = now()->toDateTimeString();
            $response['api'] = $this->responses;
            $path = storage_path('logs/seamless/login_blaze_'.now()->format('Y_m_d').'.log');
            file_put_contents($path, print_r($response, true), FILE_APPEND);
...
//                MemberProxy::where('code', $member->code)->update(['session_page' => $sessionKey]);
                $member->session_page = $sessionKey;
                $member->saveQuietly();
