...
use Gametech\Game\Services\GameListSyncService;
...
class GameUserRepository extends Repository
{
    ...
    public function gameList_($game_id, $product_id, $debug = false): array
    {
        $result['success'] = false;
    ...
    {
        $result = ['success' => false, 'msg' => 'ไม่สามารถดึงข้อมูล รายการเกมได้'];

        $game_id   = ucfirst(preg_replace('/\d/', '', (string)$game_id));
        $productId = strtoupper(trim((string)$product_id));

        $repoClass = "Gametech\\Game\\Repositories\\Games\\{$game_id}Repository";
        $repoPath  = base_path("packages/Gametech/Game/src/Repositories/Games/{$game_id}Repository.php");

        if (is_file($repoPath)) {
            $games = $this->ensureFreshList($repoClass, $productId, $debug);
            if (!empty($games)) {
                $result['success'] = true;
                $result['games']   = $games;
                $result['msg']     = 'OK';
    ...
    protected function ensureFreshList(string $repoClass, string $productId, bool $debug = false): array
    {
        /** @var GameListSyncService $sync */
        $sync      = app(GameListSyncService::class);
        $productId = strtoupper(trim($productId));

        $ttlSync   = (int) env('GAME_SYNC_TTL_SEC', 600);        // TTL สำหรับซิงก์ (10 นาที default)
        $ttlCache  = (int) env('GAME_LIST_CACHE_TTL_SEC', 600);  // TTL สำหรับแคชลิสต์เกม (10 นาที default)

        // ถ้าเกิน TTL → ค่อยลอง sync (กันชนกันด้วย lock)
        if ($sync->shouldSync($productId, $ttlSync)) {
            $sync->withSyncLock($productId, 10, function () use ($repoClass, $productId, $debug, $sync) {
                try {
                    // เรียก repo ของค่ายเพื่อดึง API และ sync ลง Mongo
                    app($repoClass, ['method' => $this->gameMethod, 'debug' => $debug])->gameList($productId);

                    // Mark เวลา sync ล่าสุด + ล้าง cache
                    $sync->markSyncedNow($productId);
                    Cache::forget($sync->cacheKey($productId));
                } catch (\Throwable $e) {
                    \Log::warning('Game sync failed', [
                        'product' => $productId,
                        'error'   => $e->getMessage(),
                    ]);
                }
        ...
        // สุดท้าย → คืนข้อมูลจาก cache/DB
        return $sync->getCachedList($productId, $ttlCache);
    }

    public function incrementClick(string $productId, string $code): void
    {
        $productId = Str::upper($productId);

        GameListProxy::where('product', $productId)
            ->where('code', $code)
            ->increment('click', 1);
    ...
    public function findGameName(string $productId, string $code): ?string
    {
        $row = GameListProxy::where('product', strtoupper($productId))
            ->where('code', $code)
            ->first(['name']);
    ...
    public function getUser($id, $getall = false, $getturn = false, $withdraw = false)
    {
        ...
        $results = $this->gameRepository->orderBy('sort')->findWhere(['status_open' => 'Y', 'enable' => 'Y', ['filepic', '<>', '']], ['code', 'id', 'filepic', 'game_type', 'name']);
            ...
            if (! is_null($game_user)) {
                ...
                if ($getturn) {
                    if ($game_user->pro_code > 0) {
                        if ($withdraw) {
                            $turn_balance = floatval($game_user->amount_balance);
    ...
    public function autoLoginSeamless($member_code, $product_id, $game_code, $debug = false): array
    {
        ...
        if (! is_null($games) && ! is_null($user)) {
            if (is_file(base_path('packages/Gametech/Game/src/Repositories/Games/'.ucfirst($games->method).'Repository.php'))) {
                return app('Gametech\Game\Repositories\Games\\'.ucfirst($games->method).'Repository', ['method' => $this->gameMethod, 'debug' => $debug])->login(['username' => $user->user_name, 'productId' => $product_id, 'gameCode' => $game_code, 'password' => $user->user_pass, 'gamelist' => $game->gamelist]);
    ...
    public function autoLoginSingle($member_code, $product_id, $game_code, $provider, $debug = false): array
    {
        ...
        if (! is_null($game) && ! is_null($user)) {
            if (is_file(base_path('packages/Gametech/Game/src/Repositories/Games/'.$game_id.'Repository.php'))) {
                return app('Gametech\Game\Repositories\Games\\'.$game_id.'Repository', ['method' => $this->gameMethod, 'debug' => $debug])->login(['username' => $user->user_name, 'password' => $user->user_pass, 'productId' => $product_id, 'gameCode' => $game_code, 'gamelist' => $game->gamelist]);
    ...
    public function gameListSingle($game_id, $product_id, $debug = false): array
    {
        $result['success'] = false;
        $result['msg'] = 'ไม่สามารถดึงข้อมูล รายการเกมได้';
        $game_id = preg_replace('/\d/', '', $game_id);
    ...
    public function GameLog($data): array
    {
        if (is_file(base_path('packages/Gametech/Game/src/Repositories/Games/SeamlessRepository.php'))) {
            return app('Gametech\Game\Repositories\Games\SeamlessRepository', ['method' => $this->gameMethod, 'debug' => false])->gameLog($data);
        }
