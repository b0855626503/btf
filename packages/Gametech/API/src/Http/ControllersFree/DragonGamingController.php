...
class DragonGamingController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        $param = [];
        $session = $request->all();
        if ($session['productId'] != 'DRAGONGAMING') {
            $param = [
                'id' => $session['id'],
                'statusCode' => 40003,
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'DRAGONGAMING')
                    ->where('response', 'in')
...
//                        ->where('con_3', $item['playInfo'])
                    ...
                    if ($datasub) {
                        ...
                        $datasub_s = GameLogProxy::where('company', 'DRAGONGAMING')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'refundsub')
                            ->where('con_1', $item['gameCode'])
...
//                            ->where('con_3', $item['id'])
                        ...
                        if ($datasub_s) {
                            ...
                            if ($datasubs) {
                                ...
                                $datasubss = GameLogProxy::where('company', 'DRAGONGAMING')
                                    ->where('response', 'in')
                                    ->where('game_user', $member->user_name)
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'DRAGONGAMING')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
                        ->where('con_1', $item['id'])
