...
class MicroGaminngNewController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'MICRO')
                    ->where('response', 'in')
...
//                                    ->orderByDesc('id')
                            ...
                            $checkDatas = GameLogProxy::where('company', 'MICRO')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->whereIn('method', ['betsub', 'ajsub'])
                                ->whereNotNull('con_1')
...
//                    $amount += $item['payoutAmount'];
                    ...
                    if ($datasub) {
                        ...
                        if ($datasubs) {
                            ...
                            $datasubss = GameLogProxy::where('company', 'MICRO')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'paysub')
                                ->where('con_1', $item['id'])
