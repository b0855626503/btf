...
class AsiaGamingController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'AGGAME')
                    ->where('response', 'in')
        ...
        $path = storage_path('logs/seamless/AGGAME' . now()->format('Y_m_d') . '.log');
...
//                    $amount += $item['betAmount'];
                    ...
                    $data_sub = GameLogProxy::where('company', 'AGGAME')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                    ...
                    if ($data_sub) {
                        ...
                        $datasub = GameLogProxy::where('company', 'AGGAME')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'betsub')
                            ->where('con_1', $item['id'])
...
//                    $amount += $item['payoutAmount'];
                    ...
                    $id = Str::of($item['id'])->explode('|');

                    if ($id->count() > 1) {


                        $chk = $id->each(function ($isub, $key) use ($member, $item) {
                            $datasub = GameLogProxy::where('company', 'AGGAME')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'betsub')
...
//                                $member->save();
                        ...
                        if ($datasub) {
                            ...
                            $datasub_s = GameLogProxy::where('company', 'AGGAME')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'refundsub')
                                ->where('con_1', $item['id'])
                            ...
                            if ($datasub_s) {
                                ...
                                $datasubs = GameLogProxy::where('company', 'AGGAME')
                                    ->where('response', 'in')
                                    ->where('game_user', $member->user_name)
                                    ->where('method', 'paysub')
                                    ->where('con_1', $item['id'])
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    if ($datasub) {
                        ...
                        if ($datasubs) {
                            ...
                            $datasubss = GameLogProxy::where('company', 'AGGAME')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'betsub')
                                ->where('con_1', $item['id'])
...
//                                $member->balance_free += $datasubss['amount'];
                                MemberProxy::where('user_name', $session['username'])->increment('balance_free', abs($datasubss['amount']));
                                $member = MemberProxy::where('user_name', $session['username'])->first();
//                                $member->save();
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'AGGAME')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
                        ->where('con_1', $item['id'])
...
//                            $datasub2->save();
                            ...
                            GameLogProxy::where('company', 'AGGAME')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'paysub')
                                ->where('con_1', $item['id'])
