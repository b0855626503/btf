...
class CockfightController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        $param = [];
        $session = $request->all();
        if ($session['productId'] != 'COCKFIGHT') {
            $param = [
                'id' => $session['id'],
                'statusCode' => 40003,
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'COCKFIGHT')
                    ->where('response', 'in')
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    if ($datasub) {
                        ...
                        if ($datasubs) {
                            ...
                            $datasubss = GameLogProxy::where('company', 'COCKFIGHT')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'betsub')
                                ->where('con_1', $item['id'])
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'COCKFIGHT')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
                        ->where('con_1', $item['id'])
