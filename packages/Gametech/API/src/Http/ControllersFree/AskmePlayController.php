...
class AskmePlayController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        $param = [];
        $session = $request->all();
        if ($session['productId'] != 'ASKMEPLAY') {
            $param = [
                'id' => $session['id'],
                'statusCode' => 40003,
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'ASKMEPLAY')
                    ->where('response', 'in')
...
//                        ->whereNull('con_4')
                    ...
                    if ($datasub) {
                        ...
                        $datasub_s = GameLogProxy::where('company', 'ASKMEPLAY')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'refundsub')
//                            ->where('con_1', $item['id'])
...
//                            ->where('con_3', $item['txnId'])
                        ...
                        if (count($datasubs) > 0) {
                            ...
                            $balance = ($member->balance_free - $sumamount);
                            if ($balance >= 0) {
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'ASKMEPLAY')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
//                        ->where('con_1', $item['id'])
