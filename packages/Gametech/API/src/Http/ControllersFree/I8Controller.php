...
class I8Controller extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'I8')
                    ->where('response', 'in')
        ...
        $path = storage_path('logs/seamless/I8' . now()->format('Y_m_d') . '.log');
...
//                    $amount += $item['betAmount'];
                    ...
                    $data_sub = GameLogProxy::where('company', 'I8')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                    ...
                    if ($data_sub) {
                        ...
                        $datasub = GameLogProxy::where('company', 'I8')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'betsub')
                            ->where('con_1', $item['gameCode'])
...
//                                $balance += $item['payoutAmount'];

                            } else {
                                $balance = ($member->balance_free - abs($item['betAmount']) + $item['payoutAmount']);
//                                $balance += $item['payoutAmount'];
                            }
...
//                                $member->save();
                            ...
                            $session_in['con_1'] = $item['gameCode'];
                            $session_in['con_2'] = $item['roundId'];
...
//                        ->where('con_3', $item['playInfo'])
                    ...
                    if (!$datasub) {
                        ...
                        $datasubs = GameLogProxy::where('company', 'I8')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
...
//                            ->where('con_3', $item['id'])
                        ...
                        if ($datasubs) {
                            ...
                            $datasubss = GameLogProxy::where('company', 'I8')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'betsub')
                                ->where('con_1', $item['gameCode'])
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'I8')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
                        ->where('con_1', $item['id'])
