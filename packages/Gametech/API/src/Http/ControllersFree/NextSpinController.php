...
class NextSpinController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'NEXTSPIN')
                    ->where('response', 'in')
...
//                            ->where('con_3', $item['txnId'])
                        ...
                        if ($datasubs) {
                            ...
                            $datasubss = GameLogProxy::where('company', 'NEXTSPIN')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'betsub')
//                                ->where('con_1', $item['id'])
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'NEXTSPIN')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
...
//                            $datasub2->save();
                        ...
                        if ($datasubss) {
                            ...
                            $balance = ($member->balance_free - $datasubss['amount']);

                            if ($balance >= 0) {

                                MemberProxy::where('user_name', $session['username'])->decrement('balance_free', $datasubss['amount']);
                                $member = MemberProxy::where('user_name', $session['username'])->first();
