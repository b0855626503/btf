...
class AskmePlayNewController extends AppBaseController
{
    protected $_config;
...
//                            ->where('con_3', $item['txnId'])
                    ...
                    $checkDatass = GameLogProxy::where('company', $this->game)
                        ->where('response', 'in')
                        ->where('game_user', $this->member->user_name)
                        ->where('method', 'betsub')
//                            ->where('con_1', $item['id'])
...
//                        ->whereNull('con_4')
                    ...
                    if(count($checkDatass) > 0){
                        foreach ($checkDatass as $items) {
                            $sumamount -= $items['amount'];
                        }
                    }


                    $balance = ($this->member->balance_free - $sumamount);
                    if ($balance >= 0) {
...
//                        }
                        ...
                        foreach ($checkDatass as $checkData1) {
