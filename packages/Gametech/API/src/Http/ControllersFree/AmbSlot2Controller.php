...
class AmbSlot2Controller extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'AMBSLOT2')
                    ->where('response', 'in')
            ...
            if ($data) {
                ...
                if ($turnover == 0) {
                    ...
                    if ($amount == 0) {
                        ...
                        $session_in['amount'] = $amount - $bet;
                    }
                } else {
                    $session_in['amount'] = (($bet - $turnover) + $amount);
                }
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    if ($datasub_s) {
                        ...
                        if ($datasubs) {
                            ...
                            $datasubss = GameLogProxy::where('company', 'AMBSLOT2')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                            ...
                            if ($datasubss) {
                                ...
                                if ($datamain->type === 'SLOT' || $datamain->type === 'ARCADE') {
                                    ...
                                    if ($amounts >= 0) {
                                        $bet = (isset($item['betAmount']) ? $item['betAmount'] : 0);
...
//                                        $member->save();
                                    ...
                                    $datasubsss = GameLogProxy::where('company', 'AMBSLOT2')
                                        ->where('response', 'in')
                                        ->where('game_user', $member->user_name)
                                        ->where('method', 'betsub')
                                        ->where('con_1', $item['id'])
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'AMBSLOT2')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
                        ->where('con_1', $item['id'])
