...
class Ace333Controller extends AppBaseController
{
    protected $_config;
...
//                    $amount += $item['betAmount'];
                    ...
                    $data_sub = GameLogProxy::where('company', 'ACE333')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
...
//                        ->where('con_3', $item['playInfo'])
                    ...
                    if ($datasub) {
                        ...
                        $datasub_s = GameLogProxy::where('company', 'ACE333')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'refundsub')
                            ->where('con_1', $item['gameCode'])
...
//                            ->where('con_3', $item['id'])
                        ...
                        if ($datasub_s) {
                            ...
                            if ($datasubs) {
                                ...
                                $datasubss = GameLogProxy::where('company', 'ACE333')
                                    ->where('response', 'in')
                                    ->where('game_user', $member->user_name)
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'ACE333')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
                        ->where('con_1', $item['id'])
