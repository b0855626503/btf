...
class AmbSportBookNewController extends AppBaseController
{
    protected $_config;
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    if ($item['status'] === 'OPEN') {
                        ...
                        if ($item['skipBalanceUpdate'] !== true) {
...
//                    $member = MemberProxy::where('user_name', $session['username'])->first();
                    ...
                    GameLogProxy::where('con_4', 'bet_' . $checkBet['_id'])
                        ->where('company', $this->game)
                        ->where('response', 'in')
                        ->where('game_user', $this->member->user_name)
                        ->update(['con_4' => 'settle_' . $id]);
...
//                        ->where('amount', $item['payoutAmount'])
                    ...
                    $balance = ($this->member->balance_free - $checkData['amount']);
...
//
    ...
    public
    function adjustBets(Request $request)
    {
        $param = [];
        $amount = 0;
        $session = $request->all();
    ...
    public
    function winRewards(Request $request)
    {
        $param = [];
        $amount = 0;
        $session = $request->all();
...
//                    $amount += $item['payoutAmount'];
    ...
    public
    function placeTips(Request $request)
    {
        $param = [];
        $amount = 0;
        $session = $request->all();
    ...
    public
    function cancelTips(Request $request)
    {
        $param = [];
        $amount = 0;
        $session = $request->all();
