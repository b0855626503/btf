...
class PragmaticPlaySlotController extends AppBaseController
{
    protected $_config;
    ...
    public function transaction(Request $request)
    {
        $param = [];
        $session = $request->all();
        if ($session['productId'] != 'PRAGMATIC_SLOT') {
            $param = [
                'id' => $session['id'],
                'statusCode' => 40003,
        ...
        if ($member) {
            ...
            foreach ($session['txns'] as $item) {
                ...
                $data = GameLogProxy::where('company', 'PRAGMATIC_SLOT')
                    ->where('response', 'in')
...
//                        ->where('con_3', $item['playInfo'])
                    ...
                    if ($datasub) {
                        ...
                        $datasubs = GameLogProxy::where('company', 'PRAGMATIC_SLOT')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'betsub')
//                            ->where('con_1', $item['id'])
...
//                        ->where('con_3', $item['playInfo'])
                        ...
                        if ($datasubs) {
                            ...
                            $datasubss = GameLogProxy::where('company', 'PRAGMATIC_SLOT')
                                ->where('response', 'in')
                                ->where('game_user', $member->user_name)
                                ->where('method', 'refundsub')
//                            ->where('con_1', $item['id'])
        ...
        if ($member) {
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    $datasub2 = GameLogProxy::where('company', 'PRAGMATIC_SLOT')
                        ->where('response', 'in')
                        ->where('game_user', $member->user_name)
                        ->where('method', 'paysub')
                        ->where('con_1', $item['id'])
