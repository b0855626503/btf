...
class MicroGamingController extends AppBaseController
{
    protected $_config;
    ...
    public function transferOut($session)
    {
        ...
        if ($member) {
            ...
            $data = GameLogProxy::where('company', 'MICROGAMING')
                ->where('response', 'in')
...
//                ->whereNull('con_2')
            ...
            if ($data) {
                ...
                $datasub = GameLogProxy::where('company', 'MICROGAMING')
                    ->where('response', 'in')
                    ->where('game_user', $member->user_name)
                    ->where('method', 'cancel')
                    ->where('con_1', $session['txnId'])
...
//                    }
                        ...
                        GameLogProxy::where('company', 'MICROGAMING')
                            ->where('response', 'in')
                            ->where('game_user', $member->user_name)
                            ->where('method', 'bet')
                            ->where('con_1', $session['txnId'])
