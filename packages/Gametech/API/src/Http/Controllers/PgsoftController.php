...
//        $starttime = $mtime;
        ...
        $member = $this->memberRepository->findOneWhere(['session_page' => $session['sessionToken'] ,'user_name' => $username, 'enable' => 'Y']);
            ...
            foreach ($session['txns'] as $item) {
                ...
                if ($datasub) {
                    ...
                    if (!isset($item['skipBalanceUpdate'])) {
                        ...
                        if ($balance >= 0) {
                            MemberProxy::where('user_name', $username)->decrement('balance', $item['betAmount']);
                            MemberProxy::where('user_name', $username)->increment('balance', $item['payoutAmount']);
