...
class NinjaSlotNewController extends AppBaseController
{
    protected $_config;
            ...
            if ($data) {
                ...
                foreach ($session['txns'] as $item) {
                    ...
                    if (count($checkDatas) > 0) {
                        foreach ($checkDatas as $items) {
                            $sumamount += $items['amount'];

                        }

                        $amountsub = $this->member->balance - $sumamount;
                        if ($amountsub >= 0) {

                            $this->member->decrement($this->balances, $sumamount);
                    ...
                    if (count($checkData) > 0) {
                        foreach ($checkData as $items) {
                            $sumamount += $items['amount'];

                        }

                        $this->member->increment($this->balances, $sumamount);
                    }

                    if (count($checkDatas) > 0 || count($checkData) > 0) {
                        ...
                        foreach ($checkDatas as $checkData1) {
                            $checkData1->con_4 = 'cancel_' . $id;
                            $checkData1->save();
                        }
                        foreach ($checkData as $checkData2) {
                            $checkData2->con_4 = 'cancel_' . $id;
                            $checkData2->save();
                        }
...
//                            ->where('con_3', $item['txnId'])
                    ...
                    $balance = ($this->member->balance - $item['payoutAmount']);
