...
class NewCommonV2Controller extends AppBaseController
{
    use LogSeamless;
    ...
    public function settleBets(Request $request)
    {
        ...
        $amount = 0.0;
        foreach ($txns as $t) {
            $amount += (float)($t['payoutAmount'] ?? 0);
            $amount -= (float)($t['betAmount'] ?? 0);
        }
        ...
        $mainLogId = $glog->saveGameLogToRedis([
            ...
            'con_4' => null,
            'before_balance' => (float)$oldBalance,
            'after_balance' => (float)($this->member->balance + $amount),
            'date_create' => $this->now->toDateTimeString(),
            'expireAt' => $this->expireAt,
        ]);
        $finalize = function (array $out) use ($glog, $mainLogId, $gameUser, $company) {
        ...
        foreach ($txns as $txn) {
            ...
            $afterBalance = $this->member->balance;
            ...
            if ($isSingleState && $txnId !== null && $roundId !== null) {
                ...
                if (!$skipBalanceUpdate && $betAmt > 0) {
                    try {
                        $res = DB::transaction(function () use ($betAmt, $session , $oldBalance,$afterBalance ) {
                            $member = MemberProxy::where('code', $this->member->code)->lockForUpdate()->first();
                            ...
                            $member->refresh();
                            $afterBalance = $member->{$this->balances};
                            return ['ok' => true ,'before' => $oldBalance, 'after' => $afterBalance];
                        }, 1);
                        if (!$res['ok']) {
                            $param = $res['param'];
                            break;
                            ...
                            $param = $this->responseData(
                                    $session['id'] ?? null, $session['username'] ?? '', $session['productId'] ?? '', 0, $res['after']
                                ) + [
                                    'balanceBefore' => (float)$res['before'],
                                    'balanceAfter' => (float)$res['after']
                                ];
                        }
                    } catch (\Throwable $e) {
                        $param = $this->responseData(
            ...
            if (!$skipBalanceUpdate) {
                try {
                    $txRes = DB::transaction(function () use ($session, $payout,$isSingleState,$oldBalance) {
                        $member = MemberProxy::where('code', $this->member->code)->lockForUpdate()->first();
                       if($isSingleState === true){
                           $current = (float)$oldBalance;
                       }else{
                           $current = (float)$member->{$this->balances};
                       }
                        ...
                        return ['param' => $param, 'current' => $current,'after' => (float)$member->{$this->balances}];
                    }, 1);
                    ...
                    LogSeamless::log($company, $gameUser, $txn, (float)$txRes['current'], (float)$txRes['after'], $session['timestampMillis']);
                    $param = $txRes['param'];
                } catch (\Throwable $e) {
                    $param = $this->responseData(
                            $session['id'] ?? null, $session['username'] ?? '', $session['productId'] ?? '', 50001, $this->member->balance
    ...
    public function winRewards(Request $request)
    {
        ...
        foreach ($txns as $txn) {
            ...
            try {
                ...
                LogSeamless::log($company, $gameUser, $txn, $oldBalance, $txRes['member_balance'], $session['timestampMillis']);
                $param = $txRes['param'];
            } catch (\Throwable $e) {
                $param = $this->responseData($session['id'] ?? null, $session['username'] ?? '', $company, 50001, $this->member->balance) + [
                        'message' => $e->getMessage(),
    ...
    public function placeTips(Request $request)
    {
        ...
        foreach ($txns as $txn) {
            ...
            if ($skipUpdate) {
                ...
                LogSeamless::log($company, $gameUser, $txn, $oldBalance, $this->member->balance, $session['timestampMillis']);
                continue;
            }
    ...
    public function adjustBalance(Request $request)
    {
        ...
        foreach ($txns as $item) {
            ...
            try {
                ...
                LogSeamless::log($company, $gameUser, $item, $oldBalance, $txRes['member_balance'], $session['timestampMillis']);
                $param = $txRes['param'];
            } catch (\Throwable $e) {
                $param = $this->responseData($session['id'] ?? null, $session['username'] ?? '', $company, 50001, $this->member->balance) + ['message' => $e->getMessage()];
                break;
    ...
    public function cancelBets_(Request $request)
    {
        ...
        foreach ($txns as $txn) {
            ...
            try {
                ...
                LogSeamless::log($company, $gameUser, $txn, (float)$oldBalance, (float)$txRes['member_balance'], $session['timestampMillis']);
    ...
    public function cancelBets(Request $request)
    {
        ...
        foreach ($txns as $txn) {
            ...
            try {
                ...
                LogSeamless::log($company, $gameUser, $newtxn, (float)$oldBalance, (float)$txRes['member_balance'], $session['timestampMillis']);
    ...
    public function adjustBets(Request $request)
    {
        ...
        foreach ($txns as $txn) {
            ...
            try {
                ...
                LogSeamless::log($company, $gameUser, $newtxn, $oldBalance, $txResult['after_balance'], $session['timestampMillis']);
                $param = $txResult['param'];
            } catch (\Throwable $e) {
                $param = $this->responseData(
                        $session['id'] ?? null, $session['username'] ?? '', $company, 50001, (float)$this->member->balance
