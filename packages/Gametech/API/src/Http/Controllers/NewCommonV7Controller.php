...
class NewCommonV7Controller extends AppBaseController
{
    use LogSeamless;
        ...
        foreach ($txns as $txn) {
            ...
            try {
                $txResult = DB::transaction(function () use ($session, $txn, $status, $txnId, $roundId, $betAmount, $oldBalance, $company, $gameUser) {
                    ...
                    $current = (float) $this->member->{$this->balances};
                    $before = $current + $bet;
...
//                );
                ...
                DB::afterCommit(function () use ($glog, $txResult, $company, $gameUser, $txn,$oldBalance) {
                    $glog->saveGameLogToRedis($txResult['log']);
                    LogSeamless::log(
                        $company,
                        $gameUser,
