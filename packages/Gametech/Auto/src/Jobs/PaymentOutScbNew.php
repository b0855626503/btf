...
class PaymentOutScbNew implements ShouldQueue
{
        ...
        if ($chk['status'] === true) {
            ...
            if ($balance_start >= $amount) {
                ...
                if ($curl['status'] === true) {
                    ...
                    $bank_date = explode('T', $curl['data']['transactionDateTime']);
                    $time = explode('+', $bank_date[1]);
                    $newdate = $bank_date[0];
                    $newtime = $time[0];

                    $balance_stop = str_replace(',', '', $curl['data']['remainingBalance']);

                    $order->ref = $curl['data']['transactionId'];
                    $order->status = 1;
                    $order->status_withdraw = 'C';
                    $order->date_bank = $newdate;
                    $order->time_bank = $newtime;
