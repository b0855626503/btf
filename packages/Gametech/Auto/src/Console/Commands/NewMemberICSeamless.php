...
class NewMemberICSeamless extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'new_ic:list {date?}';
     ...
     * @var string
     */
    protected $description = 'Check and Refill IC Seamless to upline';
    ...
    public function handle()
    {
        ...
        $lists->chunk(50, function ($itemlist) use ($bonus, $ip, $startdate, $bar, $bonusmax, $bonusmin) {
...
//                $result = MemberIcAlias::firstOrNew(['date_cashback' => $startdate, 'member_code' => $items->upline_code, 'downline_code' => $items->member_code]);
...
//                $result->save();
                ...
                $chk = DB::table('members_ic')->whereDate('date_cashback', $startdate)->where('member_code', $items->upline_code)->where('downline_code', $items->member_code);
                if ($chk->doesntExist()) {
