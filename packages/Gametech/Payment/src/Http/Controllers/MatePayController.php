...
use App\Libraries\MatePay;
use App\Libraries\matepayPay;
...
class MatePayController extends AppBaseController
{
    protected $_config;
    ...
    public function deposit(Request $request)
    {
        ...
        Log::channel('matepay_deposit_create')->info('เริ่มสร้างรายการฝาก', [
            'debug' => 'start',
        ]);
        ...
        $min_deposit = config('payment.min_deposit');
        ...
        //                $playerPayBankName = $api->BanksKorea($member->bank_code);
        //        if ($cus_bank === false) {
        //            $return['success'] = false;
        ...
        $cType = config('payment.channel_type'); // QR Code type
        $bankCode = null;
        $notifyUrl = route('api.payment.deposit.callback');
        $apikey = config('payment.api_key');
        $orderAmount = (float) $amount;
        //        $idNo = $member->idNo; // Member ID or ID card number, can be member code or phone number
        //        $telCo = $member->telCo;
        ...
        $param = [
            ...
            //            'playerPayBankName' => trim($playerPayBankName),
    ...
    public function deposit_callback(Request $request)
    {
        ...
        Log::channel('matepay_deposit_callback')->info('Callback การฝาก', $message);
        ...
        if ($status === '1') {
            ...
            $data = [
                ...
                'detail' => $detail.' จำนวน '.$amount.' AUD อัตราคูณ 10',
                ...
                'remark_admin' => '',
                'emp_topup' => 0,
                'user_create' => 'รอระบบเติมอัตโนมัติ ทำรายการฝากเงินโดย matepayPay QR',
                'create_by' => 'SYSAUTO',
            ];
    ...
    public function withdraw_callback(Request $request)
    {
        ...
        Log::channel('matepay_withdraw_callback')->info('Callback การฝาก', $message);
        ...
        if ($status === '1') {
            ...
            $data->save();
            broadcast(new RealTimeMessage('matepayPay โอนเงินให้ลูกค้าแล้ว ID : '.$data->member_user.' จำนวนเงิน '.$amount.' รายการแจ้งถอนที่ '.$data->code));
            ...
            if ($response) {
                broadcast(new RealTimeMessage('matepay Payment โอนเงินไม่สำเร็จ ID : '.$data->member_user.' จำนวนเงิน '.$amount.' Ref ID '.$refId.' ระบบคืนยอดให้ลูกค้าแล้ว'));
                $data->remark_admin = '[Order ID :'.$transactionId.'] โอนไม่สำเร็จ และ ระบบคืนยอดแล้ว';
            } else {
                broadcast(new RealTimeMessage('matepay Payment โอนไม่สำเร็จ และระบบคืนยอดไม่ได้  ID : '.$data->member_user.' จำนวนเงิน '.$amount.' Ref ID '.$refId));
