...
class TopupController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        if (count($getbank) > 0) {
            ...
            $bankss = collect($getbank)->toArray();

            foreach ($bankss as $i => $banks) {
                $newbank[$i] = $banks;
                foreach ($bankss[$i]['banks_account'] as $item) {
                    $newbank[$i]['sort'] = $item['sort'];
                    $newbank[$i]['qrcode'] = $item['qrcode'] ? $item['qrcode'] : 'N';
                    $newbank[$i]['filepic2'] = $item['filepic'] ? $item['filepic'] : 'noimage.png';
                }
    ...
    public function ezpay_create(Request $request)
    {
        ...
        if (Cache::has('ezpay_'.$member->code)) {
            return false;
        }

        Cache::put('ezpay_'.$member->code, 'lock', now()->addSeconds(5));

        $time = time();

        $subMerId = config('app.ezpay_subid');
        $refId = 'DEP-'.$subMerId.'-'.date('is').str_pad($this->id(), 4, '0', STR_PAD_LEFT);
        $custBankCode = $api->Banks($member->bank_code);
        $custBankAcct = $member->acc_no;
        $custName = $member->name;
        $merId = config('app.ezpay_merid');
        $memberId = $member->user_name;
        $callbackUrl = $baseurl.'/ezpay/callback';
        $redirectUrl = $baseurl;
        if ($custBankCode === '500') {
            return view('wallet::customer.ezpay.noservice');
        }

        $param = [
            'refId' => trim($refId),
            'custBankCode' => trim($custBankCode),
            'custBankAcct' => trim($custBankAcct),
        ...
        $url = config('app.ezpay_apiurl').'/deposit';
        ...
        if ($response['success'] === true) {
            ...
            return view('wallet::customer.ezpay.maintenance');
        }
...
//        }
//
//        $serviceurl = 'https://'.config('app.admin_url').'.'.config('app.admin_domain_url');
...
//        $res['key'] = $key;
//        $res['key'] = $key;
//        $res['secertkey'] = $merchant_secert_key;
//        $res['url'] = $url;
//        dd($res);
