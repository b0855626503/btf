...
//Route::prefix('admin')->group(function () {
    ...
    Route::group(['middleware' => ['web'], 'namespace' => 'Gametech\Admin\Http\Controllers'], function () {
        ...
        Route::prefix('auth')->group(function () {
            ...
            ])->name('admin.2fa.setting')->middleware('auth:admin');

            Route::post('/enable', 'Google2FAController@enable2fa')->name('admin.2fa.enable')->middleware('auth:admin');

            Route::get('/activate', 'Google2FAController@reActivate')->name('admin.2fa.activate')->middleware('auth:admin');

            Route::post('/', 'Google2FAController@index')->name('2fa')->middleware(['auth:admin', '2fa']);
...
//            Route::get('/ic', 'CmdController@ic')->name('admin.ic.index');
            ...
            Route::group(['prefix' => $route['name']], function () use ($route) {
                ...
                Route::post('cancel', $route['controller'] . '@cancel')->name('admin.' . $route['name'] . '.cancel');

                Route::post('withdraw', $route['controller'] . '@withdraw')->name('admin.' . $route['name'] . '.withdraw');
            });
