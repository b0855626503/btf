...
class PomPayController extends AppBaseController
{
...
//        return view($this->_config['view'], compact('hash','pompay'));
    ...
    public function payout_callback(Request $request)
    {
        ...
        if($message['status'] !== 'Success'){
            app('Member\Member\Repositories\MemberCreditLogRepository')->create([
                'ip' => $ip,
                'credit_type' => 'D',
                'balance_before' => 0,
                'balance_after' => 0,
                ...
                'remark' => 'รายการอน PomPay  ' . $message['referenceId']. ' / รายการที่ : ' . $data->code. ' ถเอนไป '.$message['custBank'].' เลขบัญชี '.$message['custBankAcc']. ' จำนวน '.$message['amount'].' ถูก '.$message['status'].' ('.$message['remark'].') ',
                'kind' => 'POMPAY_S',
                'amount' => $message['amount'],
                'amount_balance' => 0,
                'withdraw_limit' => 0,
