...
class PapayaPayController extends AppBaseController
{
...
//        $this->middleware('api');
    ...
    public function callback(Request $request)
    {
        ...
        $path = storage_path('logs/papayapay/webhook_' . $mobile . '_' . now()->format('Y_m_d') . '.log');
        file_put_contents($path, print_r('-- CALLBACK --', true), FILE_APPEND);
        file_put_contents($path, print_r($message, true), FILE_APPEND);


        if ($message['status'] == '02_Paid' || $message['status'] == '01_Pending') {
//
//            $mcodes = explode('-',$message['data']['reference2']);
            $mcodes = Str::of($message['qrCodeTransactionId'])->between('PAY', 'X')->__toString();
            $mcode = ($mcodes * 1);
//            $who = app('Gametech\Payment\Repositories\BankHengpayRepository')->findOneWhere(['referenceNo' => $message['data']['reference2']]);
            ...
            $bank_account = app('Gametech\Payment\Repositories\BankAccountRepository')->findOneWhere(['banks' => 102, 'bank_type' => 1, 'enable' => 'Y', 'status_auto' => 'Y']);

            $bank = app('Gametech\Payment\Repositories\BankRepository')->find($bank_account->banks);

            $detail = 'PapapaPay QR ' . $message['qrPayBankRef'];
            ...
            $data = [
                ...
                'value' => $amount,
                'tx_hash' => $hash,
                'txid' => $message['qrCodeTransactionId'],
                'status' => 0,
                'ip_admin' => $ip,
                'member_topup' => $member->code,
                'remark_admin' => '',
                'emp_topup' => 0,
                'user_create' => 'รอระบบเติมอัตโนมัติ ทำรายการฝากเงินโดย PAPAYAPAY QR',
                'create_by' => 'SYSAUTO'
            ];

            $chk = app('Gametech\Payment\Repositories\BankPaymentRepository')->findOneWhere(['txid' => $message['qrCodeTransactionId']]);
            if (!$chk) {
...
//        return view($this->_config['view'], compact('hash','pompay'));
    ...
    public function payout_callback(Request $request)
    {
        ...
        $status = ['PAID' => 'C'];
        $data->remark_admin = 'REF ID :' . $message['transactionRef1'] . ' ' . $data->remark_admin;
