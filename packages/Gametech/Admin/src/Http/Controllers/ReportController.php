...
class ReportController extends AppBaseController
{
    ...
    {
        $pros = $this->promotionRepository->findWhere(['enable' => 'Y'])->whereIn('id', ['pro_newuser', 'pro_firstday', 'pro_allbonus','pro_faststart'])->pluck('name_th', 'code');
    ...
    public function rp_withdraw(RpWithdrawDataTable $rpWithdrawDataTable)
    {
        ...
        return $rpWithdrawDataTable->render($this->_config['view'], ['banks' => $banks]);
    }

    public function rp_withdraw_seamless(RpWithdrawSeamlessDataTable $rpWithdrawSeamlessDataTable)
    {
        $banks = $this->bankRepository->findWhere(['enable' => 'Y'])->pluck('name_th', 'code');
        return $rpWithdrawSeamlessDataTable->render($this->_config['view'], ['banks' => $banks]);
    }
    ...
    {
        if((now()->toTimeString() >= '00:00:00') && (now()->toTimeString() <= '02:00:00')){
            return 'ห้ามกดเวลา 00.00 - 02.00';
        }
    ...
    public function rp_deposit(RpDepositDataTable $rpDepositDataTable)
    {
        ...
        $responses = app('Gametech\Payment\Repositories\BankAccountRepository')->getAccountInAlls()->toArray();
...
//            dd($item);
            return [
                'code' => $item->code,
                'name' => $item->bank['name_th'] . ' [' . $item->acc_no . ']'
            ];
