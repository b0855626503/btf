...
class LuckyPayController extends AppBaseController
{
...
//        $this->middleware('api');
    ...
    public function callback(Request $request)
    {
        ...
        $path = storage_path('logs/luckypay/webhook_' . $mobile . '_' . now()->format('Y_m_d') . '.log');
...
//            $mcodes = explode('-',$message['data']['reference2']);
            $mcodes = Str::of($message['clientNo'])->between('LUK', 'X')->__toString();
            $mcode = ($mcodes * 1);
//            $who = app('Gametech\Payment\Repositories\BankHengpayRepository')->findOneWhere(['referenceNo' => $message['data']['reference2']]);
            ...
            $bank_account = app('Gametech\Payment\Repositories\BankAccountRepository')->findOneWhere(['banks' => 101, 'bank_type' => 1, 'enable' => 'Y', 'status_auto' => 'Y']);

            $bank = app('Gametech\Payment\Repositories\BankRepository')->find($bank_account->banks);

            $detail = 'Lucky QR ' . $message['orderNo'];
            ...
            $data = [
                ...
                'value' => $amount,
                'tx_hash' => $hash,
                'txid' => $message['clientNo'],
                'status' => 0,
                'ip_admin' => $ip,
                'member_topup' => $member->code,
                'remark_admin' => '',
                'emp_topup' => 0,
                'user_create' => 'รอระบบเติมอัตโนมัติ ทำรายการฝากเงินโดย LUCKYPAY QR',
                'create_by' => 'SYSAUTO'
            ];

            $chk = app('Gametech\Payment\Repositories\BankPaymentRepository')->findOneWhere(['txid' => $message['clientNo']]);
            if(!$chk){
...
//        return view($this->_config['view'], compact('hash','pompay'));
    ...
    public function payout_callback(Request $request)
    {
        ...
        if ($config->seamless == 'Y') {
            $data = app('Gametech\Payment\Repositories\WithdrawSeamlessRepository')->findOneWhere(['txid' => $message['clientNo']]);
        } else {
            $data = app('Gametech\Payment\Repositories\WithdrawRepository')->findOneWhere(['txid' => $message['clientNo']]);
        }

        $status = ['PAID' => 'C'];
        $data->remark_admin = 'REF ID :'.$message['orderNo'].' '.$data->remark_admin;
