...
use Gametech\Admin\DataTables\CheckCaseDataTable;
...
class CheckCaseController extends AppBaseController
{
    protected $_config;
    ...
    public function index(CheckCaseDataTable $checkCaseDataTable)
    {
        $banks =  $banks = $this->bankAccountRepository
            ->with('bank')
            ->where('bank_type', 1)
            ->where('enable', 'Y')
            ->where('slip', 'N')
            ...
            ->mapWithKeys(function ($item) {
                return [$item->banks => $item->bank->name_th];
            });
//        dd($banks);

        return $checkCaseDataTable->render($this->_config['view'], ['banks' => $banks]);
    }
