    ...
    <script>
        $(function () {
            ...
            window.LaravelDataTables["dataTableBuilder"].on('draw', function () {
                ...
                let html = '<tr>';
                html += '<th style="text-align:right;width:80%;color:blue">Stake (ทั้งหมด)</th><th style="text-align:right;color:blue;">' + table.ajax.json().stake + '</th>';
                html += '</tr>';
                html += '<tr>';
                html += '<th style="text-align:right;width:80%;color:blue">Payout (ทั้งหมด)</th><th style="text-align:right;color:blue;">' + table.ajax.json().payout + '</th>';
                html += '</tr>';
                html += '<tr>';
                html += '<th style="text-align:right;width:80%;color:blue">ส่วนต่าง (ทั้งหมด)</th><th style="text-align:right;color:blue;">' + parseFloat((parseFloat(table.ajax.json().payout.replace(/,/g, '')) - parseFloat(table.ajax.json().stake.replace(/,/g, '')))).toFixed(2) + '</th>';
                html += '</tr>';
