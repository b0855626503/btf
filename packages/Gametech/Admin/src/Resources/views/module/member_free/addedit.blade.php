...
<b-modal ref="gamelog" id="gamelog" centered size="lg" :title="caption" :no-stacking="true" :no-close-on-backdrop="true"
    ...
    <b-table striped hover small outlined show-empty v-bind:items="items" :fields="fields" :busy="isBusy"
...
<b-modal ref="money" id="money" centered size="sm" title="เพิ่ม - ลด Credit" :no-stacking="true"
    ...
    <b-form @submit="moneySubmit" v-if="show">
        ...
        <b-form-group
            ...
            label="จำนวนเงิน:"
            label-for="amount"
            description="ระบุจำนวนเงิน ระหว่าง 0 - 10,000">
            <b-form-input
                id="amount"
        ...
        window.app = new Vue({
            ...
            data() {
                return {
                    ...
                    typesmoney: [{value: 'D', text: 'เพิ่ม ยอดเงินฟรี'}, {value: 'W', text: 'ลด ยอดเงินฟรี'}],
            ...
            methods: {
                ...
                async loadData() {
                    const response = await axios.get("{{ url($menu->currentRoute.'/loaddata') }}", {  params: {
                            id: this.code
                        }});
                    this.formaddedit = {
