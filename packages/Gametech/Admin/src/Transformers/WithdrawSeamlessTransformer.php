...
class WithdrawSeamlessTransformer extends TransformerAbstract
{
...
//            'refill' => '',
            'refill' => (!is_null($model->payment_last) ? "<span class='text-success' data-html='true' data-toggle='popover' data-placement='top' data-content='วันที่เ้ติม ".$model->payment_last['date_create']."'>".$model->payment_last['bank'].' [ '.$model->payment_last['value'].' ]</span>' : ''),
            'status' => '',
...
//            'check' => '<button type="button" class="btn ' . ($model->check_status == 'Y' ? 'btn-success' : 'btn-danger') . ' btn-xs icon-only" onclick="editdata(' . $model->code . "," . "'" . core()->flip($model->check_status) . "'" . "," . "'check_status'" . ')">' . ($model->check_status == 'Y' ? '<i class="fa fa-check"></i>' : '<i class="fa fa-times"></i>') . '</button>',
            'waiting' => view('admin::module.withdraw_seamless.datatables_confirm', ['code' => $model->code, 'status' => $model->status , 'emp_code' => $model->emp_approve])->render(),
            'cancel' => view('admin::module.withdraw_seamless.datatables_cancel', ['code' => $model->code, 'status' => $model->status , 'emp_code' => $model->emp_approve])->render(),
            'delete' => view('admin::module.withdraw_seamless.datatables_delete', ['code' => $model->code, 'status' => $model->status , 'emp_code' => $model->emp_approve])->render()
        ];
    }
